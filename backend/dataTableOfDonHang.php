...
$sql = "SELECT STT,donDat.maDon,maSP,tenSP,soLuong,giaBan,tinhTrangGH,thanhTien,hinhAnh,ngayDat FROM chiTietDon INNER JOIN donDat ON chiTietDon.maDon=donDat.maDon WHERE 1=1";

if (!empty($searchValue)) {
    $sql .= " AND tenSP LIKE '%" . $searchValue . "%')";
}
...
$totalQuery = "SELECT COUNT(*) as count FROM chiTietDon";
$totalResult = $conn->query($totalQuery);
$totalRow = $totalResult->fetch_assoc();
$totalRecords = $totalRow['count'];

// Đếm tổng số hàng sau khi áp dụng điều kiện tìm kiếm
$filterQuery = "SELECT COUNT(*) as count FROM chiTietDon WHERE 1=1";
if (!empty($searchValue)) {
    $filterQuery .= " AND tenSP LIKE '%" . $searchValue . "%' ";
}
